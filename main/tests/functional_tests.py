...
def test_home_page_for_unauthenticated_user(browser, live_server):
    ...
    # Test url
    assert (
        browser.current_url == live_server.url + "/"
    ), "Home page url should be [live server url]+'/'."
    # Test Log in Link
    try:
    ...
    try:
        ...
        assert (
            False
        ), "Home page should include 'Sign Up' link for unauthenticated user."
...
@pytest.mark.django_db
def test_home_page_for_authenticated_user(
    authenticated_browser, authenticated_user, live_server
):
    # Setup
    url = live_server.url + reverse("main:home")
    ...
    assert (
        authenticated_user.username in page_source
    ), "'authenticated_user.username' should be in page source."
    # Test Log Out Link
    try:
    ...
    ), "Broswer title should be 'SITE NAME' upon logout."
